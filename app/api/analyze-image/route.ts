...
function parseSelectiveReceipt(rawText: string): ParsedReceiptItem[] {
  ...
  for (const line of lines) {
    ...
    const nameForStorage = inferredName.toLowerCase()
    const suggestedStorage: ParsedReceiptItem['suggestedStorage'] =
      nameForStorage.includes('milk') ||
      nameForStorage.includes('paneer') ||
      nameForStorage.includes('curd') ||
      nameForStorage.includes('dahi') ||
      nameForStorage.includes('yogurt') ||
      nameForStorage.includes('butter')
        ? 'refrigerator'
        : 'room_temp'
